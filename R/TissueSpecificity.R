...
#' IUTtsi
#' @description define tissue specificexpresed genes expThreshold expression threshold to apply to detect tissue-specificity
...
ts.IUT <- function(name = "exp", eset, tissues, target,
  ...
  #dir.create(paste0(out, "/", target), showWarnings=FALSE, recursive=FALSE,
  #           mode="0777")
  #-----------------------------------------------------------------------------
  ...
  t.iut <- IUTest(m.eset, factors, as.numeric(which(samples==target)),
                  threshold)
  tm.iut <- meansFactor[names(which(t.iut$TS == TRUE)), ]
  m.iut.tsi.filt <- IUTtsi(meansFactor, t.iut$TS, target, filter)
  #head(m.iut.tsi.filt)
  #m.iut.tsi.filt <- IUTtsi(meansFactor, t.iut$TS, target, (1 -threshold))
  ...
  if((length(rownames(m.iut.tsi.filt)) > 1) == TRUE){
    ...
    #print("ts.IUT- middle")
    #write.table(rankedTSI, paste0(out, "/", target, "/", name,
    #                              "_IUT_TSI_filtered.txt"), sep="\t")
    #print("ts.IUT- end")
    # Heatmap graph from IUT
    #png(paste0(out, "/", target, "_", name,
    #           "_IUT_TSI_heatmap.png"), width=800, height=750)
    #heatmap.ts(m.iut, paste0("IUT", "-", target))
    #dev.off()
  }else{ rankedTSI <- cbind(meansFactor, tsi.order = rep(0, nrow(meansFactor)))}
  
  
  m.iut.e <- meansFactor[names(which(t.iut$TE == TRUE)), ]
  ...
  if((length(rownames(m.iut.tse.filt)) > 1) == TRUE){
    ...
    #print(head(rankedTSE))
    #write.table(rankedTSE, paste0(out, "/", target, "/", name,
    #                              "_IUT_TSE_filtered.txt"), sep="\t")
    #print("ts.IUT- end")
    # Heatmap graph from IUT
    #png(paste0(out, "/", target, "_", name,
    #           "_IUT_TSI_heatmap.png"), width=800, height=750)
